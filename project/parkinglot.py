...
from mininet.node import CPULimitedHost, Switch, OVSSwitch, RemoteController
...
def run_parkinglot_expt(net, n):
    ...
    results ={}
    for i in range(1,n):
        senderN = net.getNodeByName("%s%d"%("h",i))
        senderN.cmd('iperf -c {} -p {} -t {} -i 1 -yc > {}/iperf_{}.txt'.format(recvr.IP(), 5001, seconds, args.dir, senderN))
...
def main():
    ...
    link = custom(TCLink, bw=args.bw, delay='1ms',
                  max_queue_size=200)
    # connect the remote floodlight controller
